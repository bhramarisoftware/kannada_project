...
export default function CustomPage() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/booksales-types");
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/booksales-types/${rowToDelete.id}`,
        { method: "DELETE" }
      );
      ...
      <div style={{ backgroundColor: "#f5f5f5", minHeight: "100vh" }}>
        <Box sx={{ p: 3 }}>
          ...
          <Dialog open={openForm} onClose={handleCancel} maxWidth="md" fullWidth>
            <DialogTitle>{currentRow.id ? <b>ನಿರ್ವಾಹಕ ಹಂತ ಪುಸ್ತಕ ಮಾರಾಟದ ಪ್ರಕಾರ ತಿದ್ದುಪಡಿ</b> : <b>ಹೊಸ ನಿರ್ವಾಹಕ ಹಂತ ಪುಸ್ತಕ ಮಾರಾಟದ ಪ್ರಕಾರ</b>}</DialogTitle>
