...
//         <span style={{marginLeft:28 , fontSize:"18px"}}>&lt;&nbsp;&nbsp;   Back</span> 
//       </div>
...
export default function MemberDetails() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/api/members")
      .then((res) => {
        const members = res.data.map((member, index) => ({
          id: member.memberNumber || index + 1,   //  Always give an id
          ...
          amount: member.entries?.[0]?.payment
            ? Number(member.entries[0].payment)
            : 0,  // Extract from entries
          payment: member.entries?.[0]?.paymentType || "", // Bank / Cash
        }));
        setRows(members); //  Now rows all have `id`
      })
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (filterType === "active") return row.membershipType === "ಆಜೀವ";
    if (filterType === "inactive") return row.membershipType !== "ಆಜೀವ";
    if (filterType === "books") return row.books === "";
    return true;
  });

  const columns = [
    { field: "id", headerName: "ಸದಸ್ಯರ ಸಂಖ್ಯೆ", width: 120 },
    { field: "name", headerName: "ಶ್ರೀ / ಶ್ರೀಮತಿ", width: 150 },
    { field: "nickname", headerName: "Nick name", width: 150 },
    ...
    { field: "payment", headerName: "ನಗದು ವಿವರ", width: 200 },
    {
      ...
      renderCell: (params) => (
        <IconButton
          ...
          onClick={() => {
            axios
              .delete(`http://localhost:5000/api/members/${params.row.memberId}`)
              .then(() => {
                setRows(rows.filter((row) => row.id !== params.row.id));
              })
  ...
  return (
    <>
      ...
      <div onClick={() => navigate(-1)} style={{ cursor: "pointer", backgroundColor: "#f5f5f5", paddingTop: "1%", paddingBottom: "1%" }}>
        <span style={{ marginLeft: 28, fontSize: "18px" }}>
          &lt;&nbsp;&nbsp; Back
        </span>
      </div>
      ...
      >
        ...
        >
          <div>
            <strong>ಪೂರ್ಣ ವರದಿ</strong>
            <Typography sx={{ marginTop: 2, marginBottom: 2 }}>
              ಒಟ್ಟು ಪೂರ್ಣ ವರದಿ ಸಂಖ್ಯೆ: {filteredRows.length}
            </Typography>
          </div>
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            <TextField
            ...
            >
              ...
              <MenuItem value="active">ಆಜೀವ ಸದಸ್ಯರು</MenuItem>
              <MenuItem value="inactive">ವಾರ್ಷಿಕ ಸದಸ್ಯರು</MenuItem>
              <MenuItem value="books">ಪುಸ್ತಕ ಮಾರಾಟ</MenuItem>
            </TextField>
        ...
        >
          ಒಟ್ಟು ಮೊತ್ತ: ₹{" "}
          {filteredRows.reduce((acc, row) => acc + row.amount, 0).toFixed(2)}
        </Box>
      </Box>
    </>
  );
